...
## Problem
MySQL user `a1774c4c_marrytube` is restricted to `localhost` only, but you're connecting from remote IP `157.48.90.169`.
...
3. **Find Current Users Section**
   ...
   - Look for: `a1774c4c_marrytube@localhost`
...
5. **Verify Database Privileges**
   ...
   - Make sure user `a1774c4c_marrytube` has access to database `a1774c4c_marrytube`
...
4. **Grant Database Privileges**
   - Make sure user has privileges on database `a1774c4c_marrytube`
...
2. **Create New User**
   - Username: `a1774c4c_marrytube_remote` (or same name)
   - Password: `Marrytube@123!`
   - Host: `%` (or your IP `157.48.90.169`)

3. **Add User to Database**
   - Select user: `a1774c4c_marrytube_remote`
   - Select database: `a1774c4c_marrytube`
   - Click **"Add"**
   - Grant **ALL PRIVILEGES**

4. **Update .env file**
   ```env
   DB_USER=a1774c4c_marrytube_remote
   ```
...
```sql
-- Check current users
SELECT user, host FROM mysql.user WHERE user = 'a1774c4c_marrytube';

-- Option 1: Rename user host
RENAME USER 'a1774c4c_marrytube'@'localhost' TO 'a1774c4c_marrytube'@'%';

-- Option 2: Create new user with % host
CREATE USER 'a1774c4c_marrytube'@'%' IDENTIFIED BY 'Marrytube@123!';
...
4. Try connecting with specific IP instead of `%`:
   - Create user: `a1774c4c_marrytube@157.48.90.169`
